...
(function(module) {
  ...
  Article.loadAll = articleData => {
    ...
    let mappedData = articleData.map(ele => {
      return Article.all.push(new Article(ele));
    })
  ...
  */
  ...
  Article.numWordsAll = () => {
    let total = 0;
    let final=0;
    return Article.all.map(total => {
      return {
        wordArray: total.body.split(' ').length
      }).reduce((acc, current) => acc + current)
    };
    
    {final+=total;total=0;}).reduce(total += this.body.split(' ').length)
  };
